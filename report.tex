...
\begin{titlepage}
  ...
  {\Large \textbf{HỆ THỐNG PHÁT HIỆN CHÁY RỪNG THỜI GIAN THỰC DỰA TRÊN EDGE AI TRÊN ESP32}}\\[2cm]
...
\begin{itemize}
  ...
  \item \textbf{Triển khai mô hình trên nền tảng Edge AI}: Thiết kế kiến trúc hệ thống cho phép suy luận (inference) trực tiếp trên thiết bị biên dựa trên vi điều khiển ESP32-S3 (hoặc thiết bị tương đương), đảm bảo thỏa mãn các ràng buộc về bộ nhớ, tốc độ và năng lượng.
...
\begin{itemize}
  ...
  \item Kiến trúc vi điều khiển ESP32-S3 và camera ESP32-CAM;
...
\section{Vi điều khiển ESP32-S3 và ESP32-CAM}

\subsection{Khái niệm}

ESP32-S3 là dòng vi điều khiển 32-bit của Espressif, tích hợp WiFi, hỗ trợ AI instructions (vector extensions) và có tuỳ chọn PSRAM dung lượng lớn (8 MB). ESP32-CAM là module camera tích hợp ESP32 với cảm biến OV2640, thường dùng trong các ứng dụng giám sát hình ảnh chi phí thấp.

\subsection{Đặc điểm/khả năng}

Các đặc điểm quan trọng:
\begin{itemize}
  \item Bộ xử lý hai nhân, xung nhịp tới 240 MHz;
  \item Tích hợp WiFi 2{,}4 GHz, hỗ trợ nhiều chế độ tiết kiệm năng lượng;
  \item Hỗ trợ PSRAM 8 MB, đủ để lưu trữ tensor trung gian của mô hình đã lượng tử hoá;
  \item Hỗ trợ framework ESP-IDF và thư viện TensorFlow Lite Micro.
\end{itemize}

\subsection{Chức năng trong hệ thống}

Trong hệ thống, ESP32-S3/ESP32-CAM đảm nhiệm:
\begin{itemize}
  \item Thu nhận khung hình từ camera, chuyển về kích thước 224x224 RGB;
  ...
  \item Gửi cảnh báo (MQTT/HTTP) kèm toạ độ GPS, thời gian, độ tin cậy lên trạm mặt đất.
\end{itemize}
...
\begin{itemize}
  \item ESP32 đóng vai trò \emph{publisher}, gửi thông điệp \texttt{wildfire/alerts};
...
\begin{itemize}
  \item \textbf{Thiết bị biên (Edge Device -- ESP32-S3)}: Thu nhận hình ảnh, suy luận mô hình EfficientNet-Lite0 đã lượng tử hoá, phát cảnh báo khi phát hiện cháy/khói.
  \item \textbf{Trạm mặt đất (Ground Station)}: Nhận cảnh báo qua MQTT, lưu trữ cục bộ, hiển thị bản đồ thời gian thực và chuyển tiếp dữ liệu lên đám mây.
...
Sơ đồ kiến trúc được mô tả chi tiết trong README của dự án với ba khối Edge--Ground--Cloud kết nối thông qua các giao thức mạng chuẩn (MQTT/HTTP). Phần Edge sử dụng camera OV2640 trên ESP32-CAM, xử lý ảnh 224x224 RGB, đưa qua mô hình EfficientNet-Lite0 và gửi alert nếu độ tin cậy lớn hơn 0{,}8.
...
\begin{itemize}
  ...
  \item \texttt{app/}: ứng dụng C++ xử lý video thời gian thực sử dụng TorchScript;
  \item \texttt{edge/}: mã nguồn (dự kiến) cho firmware ESP32 sử dụng ESP-IDF và TensorFlow Lite Micro;
...
\section{Ứng dụng C++ xử lý video thời gian thực}

\subsection{Mục tiêu}

Ứng dụng C++ trong thư mục \texttt{app/} cho phép:
\begin{itemize}
  \item Tải mô hình đã export sang TorchScript (\texttt{fire\_detection\_scripted.pt});
  \item Đọc luồng video từ tệp hoặc camera;
  \item Chạy suy luận từng khung hình và hiển thị overlay kết quả (nhãn, độ tin cậy);
...
Theo README:
\begin{enumerate}
  \item Export mô hình từ PyTorch sang TorchScript bằng \texttt{export\_torchscript.py};
  \item Chạy script \texttt{build.sh} trong \texttt{app/} để biên dịch ứng dụng C++;
  \item Chạy chương trình \texttt{fire\_detector} với tham số video đầu vào, đường dẫn mô hình, ngưỡng tin cậy, tuỳ chọn lưu video đầu ra.
\end{enumerate}

Ứng dụng này đóng vai trò cầu nối giữa môi trường phát triển (PC/GPU) và bước triển khai nhúng, cho phép kiểm tra và tối ưu pipeline tiền xử lý, hậu xử lý trong điều kiện thời gian thực.

\section{Thiết bị biên ESP32 và trạm mặt đất}

\subsection{Thiết bị biên}

Thư mục \texttt{edge/} mô tả khuôn mẫu cho mã nguồn ESP-IDF:
\begin{itemize}
  \item Thiết lập camera, kết nối WiFi;
  \item Tích hợp TensorFlow Lite Micro cho suy luận;
  \item Gửi cảnh báo qua MQTT với cấu hình trong file header.
\end{itemize}

Các thông số như chu kỳ chụp ảnh, ngưỡng độ tin cậy, địa chỉ broker MQTT có thể cấu hình trong các file \texttt{config.h}.
...
\begin{enumerate}
  \item Fire bắt đầu tại khu vực có ESP32-CAM giám sát;
  \item ESP32-CAM chụp khung hình và thực hiện suy luận;
...
Theo mô tả trong README, một chuỗi thời gian điển hình:
\begin{itemize}
  \item T + 0s: Cháy bắt đầu;
  \item T + 5s: ESP32-CAM chụp và suy luận (confidence 0{,}92);
...
\begin{itemize}
  ...
  \item Xây dựng pipeline từ chuẩn bị dữ liệu, huấn luyện, kiểm thử đến xuất mô hình (TorchScript/TensorFlow Lite) phục vụ triển khai nhúng;
  \item Thiết kế ứng dụng C++ xử lý video thời gian thực trên PC, làm cơ sở để kiểm chứng pipeline và tối ưu tiền xử lý/hậu xử lý;
...
\begin{itemize}
  \item Phần triển khai firmware ESP32 và tích hợp TensorFlow Lite Micro mới dừng ở mức thiết kế, cần thêm thời gian để hoàn thiện và thử nghiệm thực tế;
...
\begin{itemize}
  \item Hoàn thiện firmware ESP32-S3, tối ưu bộ nhớ và năng lượng, triển khai thực tế trên các trạm quan sát rừng;
